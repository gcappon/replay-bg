function xk = modelStepMultiMealExerciseT1D(xkm1,I,CHOB,CHOL,CHOD,CHOS,CHOH,VO2,hourOfTheDay,mP,xk,model)
% function  modelStepMultiMealExerciseT1D(xkm1,I,CHOB,CHOL,CHOD,CHOS,CHOH,hourOfTheDay,mP,xk,model)
...
% ---------------------------------------------------------------------
    ...
    te = xkm1(21);
    %tess = -
    IG = xkm1(22); %Interstitial glucose concentration (mg/dL) 
    %IGss = Gb
    ...
    xk(2) = (X + model.TS*mP.p2*(1+inc2)*(SI/mP.VI)*(xk(5)-Ipb))/(1+model.TS*mP.p2);
    
    xk(1) = (G + model.TS*(mP.SG*(1+inc1)*Gb+(RaB + RaL + RaD + RaS + RaH)/mP.VG))/(1+model.TS*(mP.SG + (1+mP.r1*risk)*xk(2)));
    xk(22) = (IG + (model.TS/mP.alpha)*xk(1))/(1+model.TS/mP.alpha);
